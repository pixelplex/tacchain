...
func (s *KeeperTestSuite) TestLiquidStake() {
	...
	s.Require().Equal(gTACMintAmt, sdk.ZeroInt())
	...
	/////////////////////////////////////////
	nasTest, err := s.app.LiquidStakeKeeper.GetNetAmountState(s.ctx)
	s.T().Logf(
		"nasTest.NetAmount: %s",
		nasTest.NetAmount,
	)
	...
	s.Require().Equal(gTACMintAmt, stakingAmt)

	/////////////////////////////////////////
	nasTest, err = s.app.LiquidStakeKeeper.GetNetAmountState(s.ctx)
	s.T().Logf(
		"nasTest.NetAmount after liquid stake: %s",
		nasTest.NetAmount,
	)

	s.T().Logf(
		"nasTest.MintRate after liquid stake: %s",
		nasTest.MintRate,
	)
	/////////////////////////////////////////
	...
	ubdGTAC := sdk.NewCoin(liquidBondDenom, math.NewInt(10000))
	gTACBalance := s.app.BankKeeper.GetBalance(
		s.ctx, s.delAddrs[0], liquidBondDenom,
	)
	gTACTotalSupply := s.app.BankKeeper.GetSupply(
		s.ctx, liquidBondDenom,
	)
	s.Require().Equal(gTACBalance,
		sdk.NewCoin(liquidBondDenom, math.NewInt(50000)))
	s.Require().Equal(gTACBalance, gTACTotalSupply)

	// liquid unstaking

	ubdTime, unbondingAmt, ubds, unbondedAmt, err := s.keeper.LiquidUnstake(
		s.ctx, types.LiquidStakeProxyAcc, s.delAddrs[0], ubdGTAC,
	)
	s.Require().NoError(err)
	...
	crumb := ubdGTAC.Amount.Sub(ubdGTAC.Amount.QuoRaw(3).MulRaw(3))
	s.Require().EqualValues(unbondingAmt, ubdGTAC.Amount.Sub(crumb))
	...
	s.Require().Equal(gTACBalanceAfter,
	...
	gtacBalanceBefore := s.app.BankKeeper.GetBalance(s.ctx, s.delAddrs[0], params.LiquidBondDenom).Amount
	...
	s.Require().NoError(
		s.liquidUnstaking(s.delAddrs[0], gtacBalanceBefore, true),
	)
...
func (s *KeeperTestSuite) TestLiquidStakeFromVestingAccount() {
	...
	vestingAccAddr := "tac10n3ncmlsaqfuwsmfll8kq6hvt4x7c8czk3d7g6"
...
func (s *KeeperTestSuite) TestShareInflation() {
	...
	// ideally, the user should get 1000 gTAC (1 * 1000 / 1)
	// gTAC to mint = gTAC supply * sent TAC / total TAC
	...
	// 4. user tx went through and the mint rate is not affected by the TAC sent by the attacker
	// gTAC to mint = 1 * 1000 / 1 = 1
